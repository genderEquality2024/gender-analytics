<template>
    <div>
        <!-- <UDashboardModal
            ...
            description="Data report for the gender equality for the school year"
            :ui="{ width: 'sm:max-w-xl' }"
        >
            
        </UDashboardModal> -->
        <UModal v-model="modalVal" fullscreen>
            <UCard
                :ui="{
            ...
            >
                <template #header>
                    <div class="flex items-center justify-between">
                        Report Preiew
                        <UButton color="gray" variant="ghost" icon="i-heroicons-x-mark-20-solid" class="-my-1" @click="$emit('closePrint')" />
                    </div>
                </template>
    ...
    methods:{
        ...
        async generatePdf(data){
            ...
            pages.forEach((elpage, index) => {
                ...
                elpage.drawText(`ASCOTS GENDER ANALYTICS DASHBOARD`, {
                    x: 20,
                    y: height - 25,
                    size: 14,
                    font:fontBold,
